...
local chapters = {
	{"Les différents modes de jeu", [[
		...
		Si la carte est « <style color=orange>en préparation</style> » cela veut dire que les services IGN sont en train de la générer avec toutes les données et options que vous avez choisi sur le site Minetest à la carte.
		Vous devez donc attendre la fin de cette génération qui peut prendre plusieurs minutes. Pour mettre à jour la liste et le statut de votre carte, cliquez sur le bouton « Mettre à jour la liste ».
		Si la carte est « <style color=yellow>prête</style> » cela veut dire que vous pouvez la télécharger depuis les serveurs IGN.
		Sélectionnez la carte dans la liste puis cliquez sur le bouton « Télécharger ». La carte est téléchargée et installée automatiquement sur votre poste. Le jeu vous demande ensuite de lui donner un nom pour finaliser l’installation.
		...
		« Cartographier en 2D » : Ce bouton lancera l’outil de cartographie IGN-kidscode. Une nouvelle fenêtre va s’ouvrir et votre carte 2D va apparaître au bout de quelques secondes.
	]]},

	{"Créer partie multijoueur", [[
		Ce mode de jeu reprend les mêmes principes que le mode solo en ce qui concerne la liste des cartes, leur mise à jour, leur téléchargement et leur installation.
		Toute carte qui se trouve dans la liste du mode solo se trouvera également dans la liste des mondes disponibles pour le mode multijoueur.
		En mode multijoueur une personne (la plupart du temps l’enseignant) sélectionne une carte et doit ensuite héberger un serveur pour permettre aux élèves de se connecter au sein du même monde.
	]]},

	{"Rejoindre partie multijoueur", [[
		Lorsqu’un élève lance le jeu il doit cliquer sur l’onglet « Rejoindre une partie multijoueur ».
		Si un serveur a été correctement lancé en suivant les étapes du chapitre « Créer partie multijoueur » les élèves le verront affiché dans la liste des serveurs disponible.
		L’élève doit d’abord choisir un pseudonyme puis cliquer sur le serveur précédemment démarré par l’enseignant(e). Le serveur s’affichera comme ceci :
